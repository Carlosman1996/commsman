...
import importlib
import inspect
from backend.models.base import BaseItem, BaseResult
...
for _, module_name, _ in pkgutil.iter_modules(__path__, __name__ + "."):
    ...
    for name, obj in inspect.getmembers(module):
        if inspect.isclass(obj) and issubclass(obj, BaseItem) and obj is not BaseItem:
            globals()[name] = obj  # Make available at package level
            __all__.append(name)

# Create a registry for dynamic loading
DATACLASS_REGISTRY = {"BaseItem": BaseItem, "BaseResult": BaseResult}
